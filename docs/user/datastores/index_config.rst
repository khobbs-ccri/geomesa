...
be used in each index, by appending them with ``:``\ s after the index name. The following example creates two simple
feature types. The first, ``sft1`` has a single Z3 index that includes the default attributes. The second simple
feature type, ``sft2``, has two Z3 indices using different geometric features, as well as an attribute index on name
which includes a secondary index on dtg.
...
.. code-block:: java
    ...
    SimpleFeatureType sft1 = SimpleFeatureTypes.createType("mySft", spec);
    ...
    SimpleFeatureType sft2 = SimpleFeatureTypes.createType("mySft", spec);    
    sft2.getUserData().put("geomesa.indices.enabled", "z3:start:dtg,z3:end:dtg,attr:name:dtg");
